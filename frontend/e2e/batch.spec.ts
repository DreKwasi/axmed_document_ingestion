...
test("a reviewer uploads a batch of documents and tracks aggregate progress", async ({ page }) => {
  ...
  await expect(page.getByText(/Batch: 2 files/i)).toBeVisible();
  await expect(page.getByText("sanova_offer_export_2026-08-03.json · new mapping")).toBeVisible();
  await expect(page.getByText("ubuntu_health_price_list_Q3-2026.json · changed source structure")).toBeVisible();
});
