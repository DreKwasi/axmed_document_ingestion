...
def test_ocr_worker_executes_langchain_when_gemini_configured(tmp_path):
    ...
    with session_factory() as session:
        ...
        assert low_confidence_document.status == "auto_rejected"
        assert low_confidence_document.failure_reason == "Material is not readable enough to use safely."
        assert len(low_confidence_attempts) == 2
        assert all(attempt.status == "completed" for attempt in low_confidence_attempts)

    with session_factory() as session:
        with pytest.raises(ValueError, match="Material is not readable enough to use safely"):
            begin_image_extraction_review(session, low_doc_id, "ocr_assisted")
