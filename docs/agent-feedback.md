...
| 2026-09-09 | mermaid-pipeline-diagram-clarity | Overloading architectural pipeline diagrams with package names, raw code blocks, and granular bullet points creates visual noise that obscures system behavior. | Model pipeline diagrams in clean Mermaid style with concise 2–3 word node titles, single-line high-level subtitles, clear decision diamonds, and generous whitespace, separating architectural flow from low-level implementation details. | applied |
...
| 2026-09-08 | source-header-redundancy | Displaying raw document types like 'pdf' in a Source Schema / System card created visual duplication with the format badge and mislabeled unstructured files as schema systems. | Place document length and external integration context directly inline with the filename in the header metadata line, reserving the metadata card strip for core business quotation terms. | applied |
...
| 2026-09-08 | failed-mapping-confidence-state | A generic missing-score fallback displayed mapping confidence as pending even after extraction had terminally failed and produced nothing to map. | Make absent metric labels status-aware so terminal upstream failure is shown as not applicable while only active work remains pending. | applied |
...
| 2026-09-09 | bounded-json-profiling | Unbounded recursive profiling of JSON payloads risked RecursionError on deeply nested trees and inflated token context on large array collections. | Use an iterative stack DFS with strict depth, max-path, and array-sample bounds; document chunking and streaming architecture for massive document scaling. | applied |
| 2026-09-09 | sqlite-concurrency-and-busy-timeout | Relying on Python sqlite3 default 5-second timeout and DEFERRED transaction locks caused database-locked errors when concurrent background tasks ran alongside uncommitted write transactions across external LLM/OCR network calls. | Configure SQLite with PRAGMA busy_timeout=60000, synchronous=NORMAL, isolation_level=None, and BEGIN IMMEDIATE; strictly commit database transactions before issuing long external network/LLM requests or opening streaming HTTP responses. | applied |
| --- | --- | --- | --- | --- |
...
| 2026-09-08 | direct-image-peer-sources | Automatically redirecting single image uploads to an unselected document detail view trapped users in a legacy comparison interstitial instead of presenting the agreed peer sources. | Immediately project OCR-assisted and Direct vision as two peer sources in the table, keep image uploads on Home, and open candidate extractions directly without interstitial comparison cards. | applied |
