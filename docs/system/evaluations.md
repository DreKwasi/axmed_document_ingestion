...
JSON extraction is evaluated through the configured semantic provider against reviewed source facts and JSONPath/value grounding. The product does not retain source-content hashes, recorded responses, or reusable mappings. Provider responses in automated tests are mocked; live evaluation remains the evidence for model quality.
