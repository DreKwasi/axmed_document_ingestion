...
7. Export terminal processing results from the backend database projection as product-oriented CSV rows without internal persistence identifiers or in-progress records.
...
Extraction confidence is a transparent 0–100 source-condition score with these factors: machine readability/native-vs-OCR recovery (30%), parser quality and cleanliness (25%), and text legibility (45%). Text legibility combines average OCR confidence with the proportion of clearly legible lines, so a glare-damaged image is penalized even when a model returns plausible products. Model output count and OCR/vision agreement do not affect this score. For an uploaded image, the OCR-assisted and direct-vision readings are exposed as two source results; because both read the same physical source, they share its source-condition confidence. The API returns the band, score, and factor explanations so the UI never presents an unexplained “lowest field band.”

OCR quality is also a pre-extraction safety gate. By default, a line must reach 0.80 OCR confidence and at least 60% of detected lines must pass. Below that document-level floor, no semantic model is invoked and the source fails with an explicit unreadable-source reason. Above it, rejected OCR lines are removed from text context and masked out of the image supplied to direct vision.

Mapping confidence is calculated per canonical leaf from direct source-path grounding, row/cell association, provenance quality, deterministic reconciliation, and explicit conflicts or ambiguity. It returns a score/band plus a deduplicated `mapping_issues` list. Every field below 100% contributes one field-level mapping issue, so a sub-100 score can never be paired with `No issues found`. The source table shows `Extraction confidence` and `Mapping confidence`; the old `Review issues` column and `lowest field band` label are removed. Product breakdown rows show mapping issue counts, and the product detail drawer groups issue messages under the affected Product, Pricing, Quantity & Packaging, Supply, or Regulatory section.

When an extracted result has no attributable field scores and no mapping issues, the UI states `No issues` instead of rendering a missing-value dash. This is an explicit issue outcome, not a fabricated numeric mapping-confidence score.
