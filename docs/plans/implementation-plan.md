...
Extraction confidence is a transparent 0–100 weighted score with these factors: machine readability/native-vs-OCR recovery (20%), parser quality and cleanliness (15%), grounded source evidence quality (15%), OCR line quality when OCR is used (15%), and agreement between independent extraction paths (35%). JSON and clean native text score highly; degraded PDFs, OCR-dependent images, parser warnings, weak OCR lines, conflicting/ungrounded evidence, and OCR/vision disagreement reduce the score. For an uploaded image, the OCR-assisted and direct-vision readings are exposed as two source results with their own confidence. The API returns the band, score, and factor explanations so the UI never presents an unexplained “lowest field band.”
